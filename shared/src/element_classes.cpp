...
board::board(quint32 board_id, quint32 user_id, QString name,
             QString description, QString link, bool is_remote)
    : m_board_id(board_id), m_user_id(user_id), m_name(std::move(name)),
      m_description(std::move(description)), m_link(std::move(link)),
      m_is_remote(is_remote) {}

list::list(quint32 list_id, quint32 board_id, QString name, QString description)
    : m_list_id(list_id), m_board_id(board_id), m_name(std::move(name)),
      m_description(std::move(description)) {}

card::card(quint32 card_id, quint32 list_id, QString name, QString description)
    : m_card_id(card_id), m_list_id(list_id), m_name(std::move(name)),
      m_description(std::move(description)) {}
...
std::string filter_response::to_json() const {
  nlohmann::json data = {
      {"type", "filter"}, {"board-id", board_id}, {"cards", tag_ids}};
