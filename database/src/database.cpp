...
void db_manager::fill_query_name_to_sql_command() {
    ...
    query_name_to_sql_command["select_user"] =
        "SELECT user_id, name FROM %1.user_signature WHERE user_id = "
        ":key_value;";

    query_name_to_sql_command["select_board"] =
        "SELECT board_id, user_id, name, description FROM %1.board_signature "
        "WHERE board_id "
        "= "
        ":key_value;";
    ...
    query_name_to_sql_command["check_user_rights"] =
        "SELECT exists (SELECT * FROM %1.board_signature WHERE board_id = "
        ":board_id AND user_id = :user_id LIMIT 1);";
...
board db_manager::select_board(quint32 id) {
    ...
    return board(name, description, board_id, user_id);
}
...
list db_manager::select_list(quint32 id) {
    ...
    QString name = data[2].toString();
    QString description = data[3].toString();
    return list(name, description, list_id, board_id);
}
...
card db_manager::select_card(quint32 id) {
    ...
    QString name = data[2].toString();
    QString description = data[3].toString();
    return card(name, description, card_id, list_id);
}
