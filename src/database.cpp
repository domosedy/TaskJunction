...
void db_manager::update_command(
    ...
    QSqlQuery query(m_database);
    query.prepare(query_name_to_sql_command["update_command"]);
    query.bindValue(":table_name", table_name);
    query.bindValue(":updating_field_name", updating_field_name);
    query.bindValue(":key_field_name", key_field_name);
...
void db_manager::delete_command(
    ...
    QSqlQuery query(m_database);
    query.prepare(query_name_to_sql_command["delete_command"]);
    query.bindValue(":table_name", table_name);
    query.bindValue(":key_field_name", key_field_name);
    query.bindValue(":key_value", key_value);
    if (!query.exec()) {
...
int main(int argc, char *argv[]) {
    ...
    //    db_manager.insert_list(1, "list_name", "test");
