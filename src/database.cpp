...
namespace database {
    ...
    void db_manager::delete_command(
        ...
        QSqlQuery call_function_query;
        call_function_query.prepare(query_name_to_sql_command["delete_function"].
                arg(table_name));
    ...
    board db_manager::select_board(unsigned int id) {
        return board(get_data(select_info_by_id(BOARD_TABLE_NAME, BOARD_PRIMARY_KEY, id)));
    }

    list db_manager::select_list(unsigned int id) {
        return std::move(list(get_data(select_info_by_id(LIST_TABLE_NAME, LIST_PRIMARY_KEY, id))));
    }

    card db_manager::select_card(unsigned int id) {
        return std::move(card(get_data(select_info_by_id(LIST_TABLE_NAME, LIST_PRIMARY_KEY, id))));
    }

    tag db_manager::select_tag(unsigned int id) {
        return std::move(tag(get_data(select_info_by_id(LIST_TABLE_NAME, LIST_PRIMARY_KEY, id))));
    }
...
int main(int argc, char *argv[]) {
    ...
    db_manager::update_command(
            LIST_TABLE_NAME, "integer", "board_id", LIST_PRIMARY_KEY,
            "10", 10
    );
