...
static std::optional<quint32> get_int_field_data(const json &data,
...
static std::optional<all_ids> get_object_ids(const json &data) {
  ...
  return all_ids{board_id.value(), list_id.value(), card_id.value(),
                 tag_id.value()};
}
...
static std::optional<update_query> parseUpdateQuery(const json &json_data) {
  ...
  return update_query{ids.value(), value_id.value(), new_value.value(),
                      value_name.value(), updated_type.value()};
}
...
static std::optional<create_query> parseCreateQuery(const json &json_data) {
  ...
  return create_query{ids.value(), parent_id.value(), value_type.value(),
...
static std::optional<get_boards_info_query>
parseGetQuery(const json &json_data) {
...
static std::optional<move_query> parseMoveQuery(const json &json_data) {
  ...
  return move_query{ids.value(), old_list_id.value(), new_list_id.value(),
                    new_index.value()};
}
...
static std::optional<filter_query> parseFilterQuery(const json &json_data) {
  ...
  return filter_query{tags, board_id.value(),
                      static_cast<FilterType>(filter_type.value())};
}
