...
Server::Server(quint16 port, const QMap<QString, QString> &config)
    : port(port), db(config["REMOTE_NAME"], config["POSTGRES_USER"],
                     "localhost", config["POSTGRES_PASSWORD"]) {
  ...
  if (!server->listen(QHostAddress::Any, port)) {
    rDebug()
        << QObject::tr("Unable to start server: %1").arg(server->errorString());
    std::exit(1);
  }
...
void Server::newConnection() {
  ...
  connect(clientSock, SIGNAL(request_to_database(uint, query_type)), this,
...
void Server::execute_query(uint user_id, const query_type &query) {
  ...
  if (result_code == RequestType::LOGIN) {
    ...
    executed_result =
        execute_get_query(std::get<get_boards_info_query>(query), client_id);
    ...
    auto res = execute_filter_query(std::get<filter_query>(query), client_id);
    clientSocket->sendData(res.jsoned_object);
    return;
  } else {
...
ReturnedValue Server::execute_update_query(const update_query &query,
                                           quint32 id) {
  if (!db.check_user_rights(id, query.all_id.board_id)) {
    return ReturnedValue{false, 0,
                         error{"You have no rights"}.to_json().c_str()};
  }

  auto result = db.update_command((query.updated_type + "_signature").c_str(),
                                  query.value_name.c_str(),
                                  query.new_value.c_str(), query.value_id);

  update_response response{query.all_id, query.new_value.c_str(),
                           query.value_name.c_str()};

  return result ? ReturnedValue{true, query.all_id.board_id,
                                response.to_json().c_str()}
                : ReturnedValue{false, 0,
                                error{"An error occured"}.to_json().c_str()};
}

ReturnedValue Server::execute_delete_query(const delete_query &query,
                                           quint32 id) {
  if (!db.check_user_rights(id, query.all_id.board_id)) {
    return ReturnedValue{false, 0,
                         error{"You have no rights"}.to_json().c_str()};
...
ReturnedValue Server::execute_create_query(const create_query &query,
                                           quint32 user_id) {
  if (user_id == 0) {
    return ReturnedValue{false, 0,
  ...
  if (query.value_type == "board") {
    ...
    result =
        db.insert_board(user_id, query.value_name.c_str(),
                        query.value_description.c_str(), generate_string());
    ...
    result = db.insert_list(query.parent_id, query.value_name.c_str(),
    ...
    result = db.insert_card(query.parent_id, query.value_name.c_str(),
                            query.value_description.c_str());
  ...
  create_response response{all_ids, result, query.value_type.c_str(),
                           jsoned_object};
  return result > 0
...
Server::execute_login_query(const login_query &query) {
...
ReturnedValue Server::execute_upload_query(copy_board_query query, quint32 id) {
  ...
  created_board.m_link =
      code_string(created_board.m_link, created_board.m_board_id);

  create_response result = {all_ids{created_board.m_board_id, 0, 0, 0},
...
ReturnedValue Server::execute_move_query(const move_query &query, quint32 id) {
  ...
  auto result = db.move_card(query.all_id.card_id, query.new_list_id,
                             static_cast<int>(query.new_index) - 1);
  move_response response{query.all_id, query.old_list_id, query.new_list_id,
                         query.new_index};
  rDebug() << query.all_id.card_id << ' ' << query.new_list_id << ' '
           << query.new_index << ' ' << result;
  return result ? ReturnedValue{true, query.all_id.board_id,
                                response.to_json().c_str()}
                : ReturnedValue{false, query.all_id.board_id,
                                error{"Error in moving"}.to_json().c_str()};
}

ReturnedValue Server::execute_access_query(const access_to_board &query,
                                           quint32 id) {
  ...
  if (board_link == query.link) {
    ...
    board.m_link = code_string(board.m_link, board.m_board_id);
    create_response response{all_ids{query.board_id, 0, 0, 0}, query.board_id,
                             "board", board.to_json().c_str()};
    rDebug() << id << ' ' << board.m_link;
    return ReturnedValue{true, query.board_id, response.to_json().c_str()};
  }

  return ReturnedValue{false, query.board_id,
                       error{"Link is not valid"}.to_json().c_str()};
}

ReturnedValue Server::execute_get_query(const get_boards_info_query &query,
                                        quint32 id) {
...
ReturnedValue Server::execute_filter_query(const filter_query &query,
                                           quint32 id) {
